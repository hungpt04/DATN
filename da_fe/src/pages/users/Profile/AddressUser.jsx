...
export default function AddressUser() {
  ...
  const getDiaChilenForm = async (khachId) => {
    try {
      ...
      if (Array.isArray(addresses) && addresses.length > 0) {
        setListDiaChi(addresses);
      } else {
        setListDiaChi([]); // Đặt lại danh sách địa chỉ nếu không có địa chỉ nào
      }
  ...
  const handleSubmitDiaChi = async (e) => {
    ...
    try {
      const response = await fetch(diaChiData.id
        ? `http://localhost:8080/api/dia-chi/update/${diaChiData.id}`
        : 'http://localhost:8080/api/dia-chi/add',
        {
          method: diaChiData.id ? "PUT" : "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
      ...
      if (diaChiData.id) {
        setListDiaChi((prevList) =>
          prevList.map((item) =>
            item.id === diaChiData.id ? result : item // Thay thế địa chỉ được sửa
          )
        );
      } else {
        // Nếu thêm địa chỉ mới
        setListDiaChi((prevList) => [...prevList, result]);
      }
      ...
      swal("Lỗi!", "Có lỗi xảy ra khi thêm/cập nhật địa chỉ!", "error");
    }
  }
  ...
  const handleSetDefault = async (id) => {
    try {
      ...
      if (response.status === 200) {
        swal({
          title: "Thành công!",
          text: "Thiết lập địa chỉ mặc định thành công!",
          icon: "success",
          buttons: false,
          timer: 2000,
        });
  ...
  const handleOpen = async (address) => {
    if (address) {
      ...
      try {
        // Tìm tỉnh theo tên
        const province = provinces.find((prov) => prov.ProvinceName === address.idTinh);
        if (province) {
          setSelectedProvince(province.ProvinceID);

          // Fetch districts theo tỉnh
          const districtsResponse = await fetchDistricts(province.ProvinceID);
          setDistricts(districtsResponse || []);

          // Tìm huyện theo tên
          const district = districtsResponse?.find((dist) => dist.DistrictName === address.idHuyen);
          if (district) {
            setSelectedDistrict(district.DistrictID);

            // Fetch wards theo huyện
            const wardsResponse = await fetchWards(district.DistrictID);
            setWards(wardsResponse || []);

            // Tìm xã theo tên
            const ward = wardsResponse?.find((w) => w.WardName === address.idXa);
            if (ward) {
              setSelectedWard(ward.WardCode);
            }
          } else {
            console.error("District not found for:", address.idHuyen);
          }
        } else {
          console.error("Province not found for:", address.idTinh);
        }
      } catch (error) {
        console.error("Error in handleOpen:", error);
        Swal.fire({
