...
export default function UserProfile() {
  ...
  const handleUpdateCustomer = async (event) => {
    ...
    if (!khachHang.email.trim()) {
      ...
      check++;
    } else if (!/\S+@\S+\.\S+/.test(khachHang.email.trim())) {
      newErrors.email = '*Địa chỉ email không hợp lệ';
      check++;
    } else {
      // Kiểm tra email đã tồn tại
      const emailExists = await checkMail(khachHang.email);
      if (emailExists && khachHang.email !== initialEmail) {
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <input
              type="text"
              name="email"
              className={`w-full px-4 py-2 border rounded outline-blue-500 hover:border-gray-700 ${errorsKH.email ? "focus:outline-red-500 border-red-500 hover:border-red-600" : ""}`}
              value={khachHang.email}
              onChange={(event) => {
                handleInputChange(event)
            ...
            />
            {errorsKH.email && <p className="text-sm" style={{ color: "red" }}>{errorsKH.email}</p>}
          </div>
