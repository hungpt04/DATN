...
// //         if (formData.avatar) {
// //             formDataToSend.append('avatar', formData.avatar); // Assuming avatar is a file
// //         }
...
// //                         `http://localhost:8080/api/khach-hang/update/${id}`,
// //                         {
// //                             method: 'PUT',
...
function EditCustomer() {
    ...
    //             console.log('dia chi Tinh: ', address.idTinh);
    ...
    //                 address.districts = districts;
    //             }
    //             const district = districts?.find((dist) => dist.DistrictName === address.idHuyen);
    //             if (district) {
    ...
    const loadDiaChi = async (initPage, id) => {
        try {
            ...
            const addresses = await Promise.all(response.data.content.map(async (item) => {
                ...
                // Nếu tìm thấy province, load districts
                if (province) {
                    ...
                    // Nếu tìm thấy district, load wards
                    if (district) {
    ...
    const handleUpdateCustomer = async (e) => {
        ...
        swal({
            ...
            if (willConfirm) {
                try {
                    const response = await fetch(`http://localhost:8080/api/khach-hang/update/${id}`, {
                        method: 'PUT',
                        body: formDataToSend, // Send FormData
                    });
    ...
    const onUpdateDiaChi = (diaChiaa, index) => {
        ...
        setAddressErrors((prevErrors) => ({
            ...prevErrors,
            [index]: errors, // Lưu lỗi cho địa chỉ cụ thể
        }));
